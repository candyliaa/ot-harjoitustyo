...
class TestBall(unittest.TestCase):
    ...
    def test_ball_resets_when_leaves_right(self):
        self.ball.position.x = self.config.window_size[0] + 2 * self.ball.size
        self.ball.update(self.config.window_size)
        self.assertEqual(self.ball.position, pygame.math.Vector2(self.config.window_size[0]//2 - self.ball.speed, self.config.window_size[1]//2 + self.ball.speed))
